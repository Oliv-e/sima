<x-app-layout>
    ...
    <div class="pt-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="max-w-xl">
                    <form id="formId" action="{{route('quote.update', $data->id)}}" method="POST">
                        @csrf
                        @method('POST')
                        <div class="my-2">
                            <x-input-label for="quote1" :value="__('Baris Pertama')" />
                            <x-text-input required id="quote1" type="text" name="quote1" value="{{old('quote1',$data->quote1)}}" class="mt-1 bg-green-100 block w-full" />
                        </div>
                        <div class="my-2">
                            <x-input-label for="quote2" :value="__('Baris Kedua')" />
                            <x-text-input required id="quote2" type="text" name="quote2" value="{{old('quote2',$data->quote2)}}" class="mt-1 bg-green-100 block w-full" />
                        </div>
                        <div class="my-2">
                            <x-input-label for="quote3" :value="__('Baris Ketiga')" />
                            <x-text-input required id="quote3" type="text" name="quote3" value="{{old('quote3',$data->quote3)}}" class="mt-1 bg-green-100 block w-full" />
                        </div>
