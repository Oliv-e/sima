<x-app-layout>
    ...
    <div class="pt-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="w-full">
                    <form action="{{route('keuangan.create')}}" method="POST" id="addDataForm" enctype="multipart/form-data">
                        @csrf
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                            <div>
                                <x-input-label for="tanggal" :value="__('Tanggal')" />
                                <x-text-input id="tanggal" type="date" name="tanggal" :value="old('tanggal', \Carbon\Carbon::today()->toDateString())" class="mt-1 block w-full" />
                            ...
                            <div>
                                ...
                                <x-text-input id="keterangan" type="text" name="keterangan" class="mt-1 block w-full" />
                                <x-input-error :messages="$errors->get('keterangan')" class="mt-2" />
                            </div>

                            <div class="">
                                <label for="saldo" class="block text-sm font-medium text-gray-700">Saldo</label>
                                <input placeholder="cth Rp. 100.000" type="text" id="saldo" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-gray-700 focus:ring-gray-500" oninput="formatRupiahJurnal(this)">
                                <input type="hidden" name="saldo" id="saldo_numeric">
                            </div>
                        </div>
                        <div class="mt-4">
                            <button type="submit" id="submitBtn" class="bg-green-600 hover:bg-green-700 text-white font-semibold px-4 py-2 rounded-md">
                                Tambah Data
                            </button>
                        </div>
                    </form>
